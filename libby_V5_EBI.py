...
SYSTEM_PROMPT = """You are Libby V5, an ultra-concise enterprise data retrieval system.
...
- Always show the formula if calculating: e.g. $6,750 + $4,800 = $11,550
- Format currency: $1,234.56
- Format percentages: 12.5%
- Format large numbers with commas: 1,234
- If multiple values exist, list them as: Label: $value
- If data is not found: "Not found."
- Never use full sentences. Never explain. Never pad."""

# ─────────────────────────────────────────────
# THEMES — Clean Professional
# ─────────────────────────────────────────────
THEMES = {
    "dark": {
        "BG_DARK":       "#1c1c1e",   # Near black
        "BG_PANEL":      "#2c2c2e",   # Dark gray panel
        "BG_INPUT":      "#1c1c1e",   # Input bg
        "BG_BUBBLE_U":   "#3a3a3c",   # User bubble — dark gray
        "BG_BUBBLE_L":   "#2c2c2e",   # Libby bubble
        "BG_SIDEBAR":    "#1c1c1e",   # Sidebar
        "ACCENT":        "#e5e5ea",   # Light silver accent
        "ACCENT_DIM":    "#636366",   # Dimmer gray
        "TEXT_MAIN":     "#f2f2f7",   # Near white
        "TEXT_DIM":      "#636366",   # Muted gray
        "TEXT_SOURCE":   "#aeaeb2",   # Silver source
        ...
        "BG_CALC":       "#3a3a3c",
        "TEXT_CALC":     "#f2f2f7",
        "TEXT_BUBBLE_U": "#f2f2f7",   # Light text on dark bubble
        "TEXT_BUBBLE_L": "#f2f2f7",   # Light text on dark bubble
    },
    "light": {
        "BG_DARK":       "#f2f2f7",   # iOS-style light gray
        "BG_PANEL":      "#ffffff",   # White panel
        "BG_INPUT":      "#f2f2f7",   # Input bg
        "BG_BUBBLE_U":   "#e5e5ea",   # Light gray user bubble
        "BG_BUBBLE_L":   "#f2f2f7",   # Very light libby bubble
        "BG_SIDEBAR":    "#f2f2f7",   # Sidebar
        "ACCENT":        "#1c1c1e",   # Near black accent
        "ACCENT_DIM":    "#3a3a3c",   # Dark gray button
        "TEXT_MAIN":     "#1c1c1e",   # Near black text
        "TEXT_DIM":      "#8e8e93",   # Medium gray
        "TEXT_SOURCE":   "#636366",   # Dark gray source
        "SUCCESS":       "#34c759",   # Green
        "WARNING":       "#ff9500",   # Orange
        "BORDER":        "#d1d1d6",   # Light border
        "BTN_CLEAR":     "#ffe5e5",
        "BTN_CLEAR_FG":  "#ff3b30",
        "BG_CALC":       "#e5e5ea",   # Light gray calc bubble
        "TEXT_CALC":     "#1c1c1e",   # Dark text on light calc bubble
        "TEXT_BUBBLE_U": "#1c1c1e",   # Dark text on light gray bubble
        "TEXT_BUBBLE_L": "#1c1c1e",   # Dark text on light bubble
    }
}
...
def perform_calculation(question, filename, sheet_name, df):
    ...
    try:
        ...
        def is_currency(col):
            return any(w in col.lower() for w in ["price", "cost", "amount", "revenue", "sales", "value", "total", "quote"])

        def fmt(val, col):
            return f"${val:,.2f}" if is_currency(col) else f"{val:,.2f}"

        def get_id(df, idx):
            id_cols = [c for c in df.columns if any(w in c.lower() for w in ["name", "sku", "id", "product", "item", "description", "number", "customer"])]
        ...
        if any(w in q for w in ["total", "sum", "how much"]):
            for col in numeric_cols:
                total = df[col].sum()
                if total != 0:
                    result_lines.append(f"{col}: {fmt(total, col)}")

        if any(w in q for w in ["average", "avg", "mean"]):
            for col in numeric_cols:
                avg = df[col].mean()
                result_lines.append(f"Avg {col}: {fmt(avg, col)}")

        if any(w in q for w in ["highest", "maximum", "max", "most", "top"]):
            for col in numeric_cols:
                max_val = df[col].max()
                max_idx = df[col].idxmax()
                result_lines.append(f"Max {col}: {fmt(max_val, col)}{get_id(df, max_idx)}")

        if any(w in q for w in ["lowest", "minimum", "min", "least"]):
            for col in numeric_cols:
                min_val = df[col].min()
                min_idx = df[col].idxmin()
                result_lines.append(f"Min {col}: {fmt(min_val, col)}{get_id(df, min_idx)}")
    ...
    try:
        response = requests.post(
            ...
            json={
                ...
                "stream":  False,
                "options": {
                    "temperature": 0.0,   # Zero — pure factual
                    "top_p":       0.9,
                    "num_predict": 150    # Hard cap — forces brevity
                }
            },
            timeout=120
        )
...
class LibbyApp:
    def __init__(self, root):
        self.root          = root
        self.ready         = False
        self.current_theme = "light"   # V5 starts in light mode
        self.T             = THEMES["light"]
        self.dot_count     = 0
        self.animating     = False
        self._chat_widgets = []

        self.root.title("Libby V5 — Enterprise Intelligence")
    ...
    def _build_ui(self):
        ...
        self.title_label = tk.Label(
            self.title_frame, text="LIBBY  V5",
        ...
        self.subtitle_label = tk.Label(
            self.title_frame,
            text="Enterprise Intelligence  •  Offline  •  Ultra Concise Mode",
        ...
        self.footer_label = tk.Label(
            self.footer,
            text="🔒  Air-gapped  •  Ultra Concise Mode  •  Sales / Inventory / PO / SO / Pricing",
            bg=T["BG_PANEL"], fg=T["TEXT_DIM"],
            font=("Courier New", 8)
        )
        ...
        self._add_libby_message(
            "LIBBY V5 — Ultra Concise Enterprise Mode.\n"
            "Numbers and amounts only. No narrative.\n"
    ...
    def _refresh_sidebar(self):
        ...
        for entry in known_files:
            ...
            tk.Label(
                row, text=f"{icon} {filename}{badge}",
    ...
    def _draw_avatar(self):
        ...
        tk.Label(c, text="V5", bg=T["BG_PANEL"], fg=T["BG_PANEL"] if self.current_theme == "dark" else T["TEXT_MAIN"],
    ...
    def _add_user_message(self, text):
        ...
        tk.Label(bubble, text=text, bg=T["BG_BUBBLE_U"],
                 fg=T["TEXT_BUBBLE_U"],
                 font=("Courier New", 11),
    ...
    def _add_libby_message(self, text, source=None, file_type=None, is_calc=False):
        ...
        label = "LIBBY V5  ⚡" if is_calc else "LIBBY V5"
    ...
    def _add_thinking_frame(self):
        ...
        tk.Label(frame, text="LIBBY V5", bg=T["BG_PANEL"], fg=T["ACCENT"],
                 font=("Courier New", 8, "bold")).pack(anchor="w", padx=4)
        self.thinking_label = tk.Label(
            frame, text="⚡  Calculating",
            bg=T["BG_PANEL"], fg=T["TEXT_DIM"],
    ...
    def _animate_dots(self):
        ...
        self.thinking_label.config(text=f"⚡  Calculating{dots}")
        self.dot_count += 1
    ...
    def _show_answer(self, answer, source, file_type, is_calc, thinking_frame):
        ...
        self._add_libby_message(answer, source=source,
                                file_type=file_type, is_calc=is_calc)
        self._scroll_to_bottom()
