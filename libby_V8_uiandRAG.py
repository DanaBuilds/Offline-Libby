...
def ask_ollama(question, chunks, calc_results, history, mode, model):
...
Answer:"""
    try:
        # Remove/raise the word limit for responses
        max_tokens = 2048 if mode == "knowledge" else 1024  # much higher than before
...
class LibbyApp:
    ...
    def _open_settings(self):
        ...
        self.settings_win.geometry(f"440x360+{rx}+{ry}")
        ...
        tk.Button(p, text="Save & Apply", bg=T["ACCENT_DIM"], fg=T["TEXT_MAIN"],
                  ...
                  command=self._save_settings).grid(row=8, column=0, sticky="w")
        tk.Button(p, text="Cancel", bg=T["BG_INPUT"], fg=T["TEXT_DIM"],
                  font=F_BTN_SM, relief="flat", padx=12, pady=6, cursor="hand2",
                  command=self._close_settings).grid(row=8, column=1, sticky="w", padx=(8,0))
    ...
    def _draw_user_bubble(self, text):
        ...
        txt = tk.Text(bubble, height=1, bg=T["BG_BUBBLE_U"], fg=T["TEXT_BUBBLE_U"],
    ...
    def _send_question(self):
        ...
        if is_report:
            ...
            def run_query():
                vague = ["that","it","those","the same","its","their","this","them"]
                search_q = question
                if any(w in question.lower().split() for w in vague) and tab_history:
                    last_user = [m for r, m in tab_history if r == "User"]
                    if last_user: search_q = f"{last_user[-1]} {question}"
                ...
                if len(tab_history) > 10:
                    self.tabs[active_tab]["history"] = tab_history[-10:]
