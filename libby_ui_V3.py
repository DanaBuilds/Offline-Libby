...
SYSTEM_PROMPT = """You are Libby V3, a strict offline data retrieval system.
...
# ─────────────────────────────────────────────
# THEMES
# Color theory notes on dark mode:
#   - Background: dark charcoal-navy #1a1a2e avoids pure black halation
#   - Panel: slightly lighter #16213e gives clear depth layering
#   - Text: warm off-white #e2e8f0 reduces eye strain vs pure white
#   - Accent: mid-bright purple #a78bfa — vivid but not harsh
#   - Bubbles: stepped lighter than BG for clear visual separation
# ─────────────────────────────────────────────
THEMES = {
    "dark": {
        "BG_DARK":       "#1a1a2e",   # Deep charcoal navy — avoids pure black halation
        "BG_PANEL":      "#16213e",   # Slightly lighter navy panel
        "BG_INPUT":      "#0f3460",   # Rich deep blue input
        "BG_BUBBLE_U":   "#2d2b55",   # Muted purple user bubble — clear separation
        "BG_BUBBLE_L":   "#1e1e3a",   # Slightly lighter than BG for Libby bubble
        "BG_SIDEBAR":    "#12122a",   # Just darker than BG for sidebar depth
        "ACCENT":        "#a78bfa",   # Soft vivid purple — readable, not harsh
        "ACCENT_DIM":    "#6d4cc7",   # Deeper purple for buttons
        "TEXT_MAIN":     "#e2e8f0",   # Warm off-white — reduces halation vs #ffffff
        "TEXT_DIM":      "#7c7ca8",   # Muted purple-gray for secondary text
        "TEXT_SOURCE":   "#a78bfa",   # Matches accent
        "SUCCESS":       "#a78bfa",   # Purple for success state
        "WARNING":       "#c4b5fd",   # Lighter purple for warnings/loading
        "BORDER":        "#2d2b55",   # Subtle purple border
        "BTN_CLEAR":     "#3d1515",   # Dark red tint for clear button
        "BTN_CLEAR_FG":  "#ff6b6b",   # Soft red text
    },
    "light": {
        "BG_DARK":       "#f5f0e8",
        "BG_PANEL":      "#ede8dc",
...
class LibbyApp:
    def __init__(self, root):
        ...
        self.root.title("Libby V3 — Offline Knowledge System")
    ...
    def _build_ui(self):
        ...
        self.title_frame = tk.Frame(self.header, bg=T["BG_PANEL"])
        self.title_frame.pack(side="left")

        self.title_label = tk.Label(
            self.title_frame, text="LIBBY  V3",
        ...
        self._add_libby_message(
            "LIBBY V3 — Strict data retrieval mode.\n"
    ...
    def _draw_avatar(self):
        ...
        tk.Label(c, text="V3", bg=T["BG_PANEL"], fg=T["ACCENT"],
    ...
    def _add_libby_message(self, text, source=None, file_type=None):
        ...
        tk.Label(outer, text="LIBBY V3", bg=T["BG_PANEL"], fg=T["ACCENT"],
    ...
    def _add_thinking_frame(self):
        ...
        tk.Label(frame, text="LIBBY V3", bg=T["BG_PANEL"], fg=T["ACCENT"],
                 font=("Courier New", 8, "bold")).pack(anchor="w", padx=6)
