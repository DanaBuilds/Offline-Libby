        ...
        response = requests.post(
            ...
            json={"model": model, "prompt": prompt, "stream": False,
                  "options": {"temperature": 0.1 if mode=="knowledge" else 0.0,
                              "top_p": 0.9,
                              "num_predict": 300 if mode=="knowledge" else 150}},
            timeout=120
        )
...
class LibbyApp:
    ...
    def _draw_user_bubble(self, text):
        ...
        tk.Label(bubble, text=text, bg=T["BG_BUBBLE_U"], fg=T["TEXT_BUBBLE_U"],
                 font=F_CHAT, wraplength=720, justify="left").pack(anchor="w")

    def _draw_libby_bubble(self, text, source=None, file_type=None):
        T     = self.T
        outer = tk.Frame(self.chat_inner, bg=T["BG_PANEL"], pady=8)
        outer.pack(fill="x", padx=20)
